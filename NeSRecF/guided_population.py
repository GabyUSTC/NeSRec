...
#################################################
estimated_click_model = click_conv(config, dataset)
##################################################

guided_population_wrt_groundtruth = []

collected_source_user = torch.from_numpy(collected_source_user).to(config['device'])
for i in range(dataset.num_items):
    #neigh prefer
    collected_source_user_neigh_prefer = estimated_click_model.get_rating_per_ui(collected_source_user, i).detach().cpu().numpy()
    collected_source_user_neigh_prefer = collected_source_user_neigh_prefer - 3.0

    temp_list = []
    positive_index = np.where(collected_source_user_neigh_prefer > 0)[0]
    sorted_positive_index = np.argsort(collected_source_user_neigh_prefer[positive_index])[::-1]
    #print(collected_source_user_neigh_prefer[positive_index][sorted_positive_index])
    for j in positive_index[sorted_positive_index]:
...
guided_population_wrt_groundtruth = torch.from_numpy(guided_population_wrt_groundtruth).to(config['device'])
####individual
guided_population_wrt_groundtruth_rating = estimated_click_model.get_rating_per_ui(
    guided_population_wrt_groundtruth[:, 0], guided_population_wrt_groundtruth[:, 1]) - 3.0
...
####
guided_population_wrt_groundtruth = guided_population_wrt_groundtruth.detach().cpu().numpy()

if config['group_size'] == 1:
    final_guided_population_wrt_groundtruth = guided_population_wrt_groundtruth
    ...
    for ri in rational_item[:num_guided_item]:
        ...
        guided_user_groups_list.append(np.hstack(guided_user_groups_list_for_ri))
