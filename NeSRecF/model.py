...
class mf(nn.Module):
    ...
    def get_rating_per_ui(self, users, items):
        final_user_emb, final_item_emb = self.get_emb()
        rating = (final_user_emb[users] * final_item_emb[items].reshape(-1, final_user_emb.shape[1])).sum(dim=1)

        return rating


    def click_forward(self, items):
        final_user_emb, final_item_emb = self.get_emb()

        rating = (final_user_emb * final_item_emb[items].reshape(-1, final_user_emb.shape[1])).sum(dim=1)
...
class gatv2_spill_var2(MessagePassing):
    ...
    def get_rating_per_ui(self, users, items):
        rating = (self.user_embedding(users) * self.item_embedding(items)).sum(dim=1)

        return rating
    def get_rating_per_ui2(self, users, items):
        rating = (self.user_embedding(users) * self.item_embedding.weight[items].unsqueeze(dim=0)).sum(dim=1)

        return rating

    def get_neigh_rating_per_ui(self, users, items):
        source_user_emb = self.user_embedding(users)
        guided_item_emb = self.item_embedding.weight[items].unsqueeze(dim=0)
        neigh_prefer_emb = source_user_emb * guided_item_emb
        rating = self.linear_trans_neigh(neigh_prefer_emb).sum(dim=1)

        return rating


class gatv2_spill_var2_wo_att(MessagePassing):
    def __init__(self, config, dataset):
        super().__init__(aggr='add')
        self.num_users = config['num_users']
        self.num_items = config['num_items']
    ...
    def click_forward(self, oi, guided_users, guided_item):
        with torch.no_grad():
            ...
            g_ui_emb = self.propagate(edge_index=self.u_trust, x=ui_emb * oi)
            g_ui_emb = self.linear_trans_neigh(g_ui_emb)
...
class simulation_mf(MessagePassing):
    ...
    def click_forward(self, oi, guided_users, guided_item):
        ...
        indi_prefer = base_prefer
        neigh_prefer = base_prefer
