...
from model import mf, diffnet, gatv2_spill_var2, gatv2_spill_var2_wo_att, simulation_mf, simulation_diffnet
...
if config['click_model_name'] == 'random':
    ...
    for gi_index, gi in enumerate(dataset.guided_items):
        for gu_index, gu in enumerate(dataset.guided_user_groups_list[gi_index]):
            ...
            if config['click_model_name'] != 'groundtruth':
...
np.save(
    join(config['data_path'] + config['data_name'] + '/',
         ...
         f"{config['filter_model_type']}_{config['click_model_name']}_choosed25_guided_user_groups_list_var2.npy"),
    choosed_guided_user_groups_list)